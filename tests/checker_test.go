...
func TestCheckDiskspace(t *testing.T) {
	t.Run("Calculation accuracy", func(t *testing.T) {
		...
		required, available, _ := utils.CheckDiskspace(tmpfile.Name())

		// Verify the calculation: required should be numLines * (utils.MAX_URL_LEN + utils.MAX_HOSTNAME + 100)
		expectedPerLine := int64(utils.MAX_URL_LEN + utils.MAX_HOSTNAME + 100)
	...
	t.Run("Exceeds flag logic", func(t *testing.T) {
		...
		required, available, exceeds := utils.CheckDiskspace(tmpfile.Name())
	...
	t.Run("Simulated disk space exceeded", func(t *testing.T) {
		...
		_, available, _ := utils.CheckDiskspace(tmpfile.Name())
		...
		maxBytesPerLine := int64(utils.MAX_URL_LEN + utils.MAX_HOSTNAME + 100)
		...
		required, availableCheck, exceeds := utils.CheckDiskspace(tmpfile2.Name())
	...
	t.Run("Nonexistent file", func(t *testing.T) {
		required, available, exceeds := utils.CheckDiskspace("/nonexistent/file/path.txt")
